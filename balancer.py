...
连接空闲超时：ts 键集 op,ttl（ttl ∈ [1,10^9] 非 bool 整数）登记空闲
时限，首配作用于既有与后续连接，同值幂等、异值报 STATE，不进入
ce/ci；凡成功建连（open/oa/fx/fr）均置 last=opened_at。tk 键集
op,cid,now，未到期（now < last+ttl）才置 last=now，返回 op,ok。tg
键集 op,cid,now，返回键序 op,cid,backend,state,opened,last,deadline：
deadline=last+ttl，state 为 A（now<deadline）或 E，查询不删除。tx
键集 op,now，按建连顺序删除到期连接并递减后端并发，排空 D 后端末连
消失则转 X、end=now，返回 op,expired（按建连序列 cid）。tk/tg/tx 的
now 纳入共用非递减时钟；未 ts 调用三者报 STATE，未知 cid 或 tk 命中
到期 cid 报 CONNECTION；close 与 dg 强关亦清理连接记录。ts/tk/tg 为
...
def run(raw):
    ...
    # 可复用。last 为最近活动时刻：成功建连时置 opened_at，tk 未到期时置
    # 其 now；空闲期限 deadline=last+idle_ttl 由 ts 登记。
    connections = {}
    ...
    queue_cfg = None
    wait_queue = deque()
    # 连接空闲时限：ts 登记后作用于既有与后续连接，未 ts 为 None；
    # 不进入 ce/ci（热加载不重置它）。
    idle_ttl = None
    last_now = None
    results = []
    ...
    for raw_op in ops:
        ...
        if op[0] == "add":
            ...
            _, ttl = op
            if idle_ttl is not None and idle_ttl != ttl:
                # 异值重配报 STATE；同值幂等。
                fail(EXIT_STATE, "STATE")
            # 首配作用于既有与后续连接（建连时均已置 last=opened_at）。
            idle_ttl = ttl
            ...
            if connection is None:
                fail(EXIT_CONNECTION, "CONNECTION")
            if now >= connection[3] + idle_ttl:
                # 命中到期连接：不再续期，报 CONNECTION。
                fail(EXIT_CONNECTION, "CONNECTION")
            # 未到期才置 last=now（now 非递减，last 只会推后）。
            connection[3] = now
            ...
            if connection is None:
                fail(EXIT_CONNECTION, "CONNECTION")
            deadline = connection[3] + idle_ttl
            # 查询不删除；now<deadline 为 A，否则 E。
            results.append(
                {
                    "op": "tg",
                    "cid": cid,
            ...
            for cid, connection in list(connections.items()):
                ...
                drain = record["drain"]
                if drain["state"] == "D" and record["conns"] == 0:
                    # 排空中后端末连消失即转 X，end 取本次 tx 的 now。
                    drain["state"] = "X"
                    drain["end"] = now
